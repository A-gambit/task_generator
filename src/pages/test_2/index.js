...
export default React.createClass({
  ...
  handleClick() {
    const {mark, correct} = this.state.answers.reduce((memo, answer, index) => {
      let correctAnswer = Array.isArray(this.state.task.tests[index + 2].correct)
        ? this.state.task.tests[index + 2].correct.join(' ')
        : this.state.task.tests[index + 2].correctKey || this.state.task.tests[index + 2].correct.toString()
